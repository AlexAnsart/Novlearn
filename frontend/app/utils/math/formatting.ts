 ...
 * Gère les 0 inutiles, les signes multiples (+ -), les coefficients 1, etc.
 */
export const cleanMathExpression = (expression: string): string => {
  ...
  // 4. GESTION DES SIGNES (Ex: + - devient -)
  ...
  // 5. NETTOYAGE FINAL DU DÉBUT DE LIGNE
  // Enlever le "+" au tout début (ex: "+ x^2" -> "x^2")
  cleaned = cleaned.replace(/^\s*\+/, '');

  // 6. Enlever les parenthèses autour des nombres positifs isolés
  cleaned = cleaned.replace(/\((\d+\.?\d*)\)/g, '$1');

  // 7. SI TOUT EST VIDE (ex: 0x -> 0 -> vide), on remet "0"
