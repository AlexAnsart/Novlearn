...
export const simplifyLatexExpression = (latex: string, variables: VariableValues): string => {
  if (!latex) return '';
  ...
  for (const key of sortedKeys) {
    ...
    safeExpr = safeExpr.replace(new RegExp(`@${key}(?![a-zA-Z0-9])`, 'g'), String(value));
  ...
  try {
    ...
    // MathJS met souvent des \cdot pour la multiplication (2 \cdot x), on préfère 2x
