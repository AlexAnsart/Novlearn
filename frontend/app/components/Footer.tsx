...
import { Twitter, Instagram, Github, Mail, Apple, Smartphone } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          ...
          <div className="space-y-4">
            ...
            <p className="text-slate-400 text-sm leading-relaxed">
              La plateforme ultime pour maîtriser les mathématiques tout en s'amusant. Progressez, suivez votre progression et défiez vos amis !
            </p>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              ...
              <FooterLink href="mailto:contact@novlearn.fr">Nous contacter</FooterLink>
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              <StoreButton icon={<Apple size={24} />} title="App Store" subtitle="Bientôt sur" />
              <StoreButton icon={<Smartphone size={24} />} title="Google Play" subtitle="Bientôt sur" />
            </div>
          </div>
        ...
        <div className="pt-8 border-t border-slate-800/50 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-slate-500">
          ...
          <div className="flex gap-6">
            <Link href="/privacy" className="hover:text-slate-300 transition-colors">Confidentialité</Link>
            <Link href="/cgu" className="hover:text-slate-300 transition-colors">CGU</Link>
            <Link href="/sitemap" className="hover:text-slate-300 transition-colors">Plan du site</Link>
          </div>
        </div>
      </div>
    </footer>
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <li>
      <Link href={href} className="text-slate-400 hover:text-indigo-400 transition-colors">
        {children}
      </Link>
    </li>
  );
...
function StoreButton({ icon, title, subtitle }: { icon: React.ReactNode; title: string; subtitle: string }) {
  return (
    <button className="flex items-center gap-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white px-4 py-2 rounded-xl transition-all w-full md:w-auto text-left group">
      ...
      <div>
        <div className="text-[10px] uppercase text-slate-500 font-bold tracking-wider">{subtitle}</div>
