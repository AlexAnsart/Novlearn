...
import { 
  ...
  Moon, Sun, Monitor, Volume2, VolumeX, Bell, BellOff, Zap, Sliders 
} from 'lucide-react';
    ...
    <Layout>
      <div className="flex-1 px-4 md:px-8 pb-12 overflow-y-auto bg-slate-900">
        <div className="max-w-4xl mx-auto pt-8 space-y-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-6" style={{ fontFamily: "'Fredoka', sans-serif" }}>
            Paramètres
          </h1>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              <section className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 md:p-8">
                ...
                <div className="space-y-4 max-w-lg">
                  ...
                  {/* Ancien MDP */}
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-300">Ancien mot de passe</label>
                ...
                <section className="bg-slate-800/50 border border-slate-700 rounded-2xl overflow-hidden">
                  ...
                  <div className="p-6 flex items-center justify-between border-b border-slate-700/50">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-xl transition-colors ${soundEnabled ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700 text-slate-400'}`}>
                        {soundEnabled ? <Volume2 size={24} /> : <VolumeX size={24} />}
                      </div>
                      <div>
                        <h3 className="text-white font-bold">Effets sonores</h3>
                        <p className="text-slate-400 text-sm">Bruitages lors des exercices</p>
                      </div>
                    </div>
                  ...
                  <div className="p-6 flex items-center justify-between border-b border-slate-700/50">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-xl transition-colors ${notifications ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-700 text-slate-400'}`}>
                        {notifications ? <Bell size={24} /> : <BellOff size={24} />}
                      </div>
                      <div>
                        <h3 className="text-white font-bold">Notifications</h3>
                        <p className="text-slate-400 text-sm">Rappels d'entraînement</p>
                      </div>
                    </div>
                  ...
                  <div className="p-6 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-xl transition-colors ${!reduceMotion ? 'bg-purple-500/20 text-purple-400' : 'bg-slate-700 text-slate-400'}`}>
                        <Zap size={24} />
                      </div>
                      <div>
                        <h3 className="text-white font-bold">Animations fluides</h3>
                        <p className="text-slate-400 text-sm">Désactiver pour réduire les mouvements</p>
                      </div>
                    </div>
                    <Switch checked={!reduceMotion} onChange={(val) => setReduceMotion(!val)} />
                  </div>
...
function ThemeCard({ active, onClick, icon, label, description }: any) {
  return (
    ...
    >
      <div className={`mb-3 transition-colors ${active ? 'text-indigo-400' : 'text-slate-400 group-hover:text-slate-200'}`}>
        {icon}
      </div>
